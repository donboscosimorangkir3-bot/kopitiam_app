...
class _CashierDashboardPageState extends State<CashierDashboardPage>
  ...
  // ✅ Filter list pesanan berdasarkan tab aktif + filter hari ini untuk completed
  List<Order> get _filteredOrders {
    final now = DateTime.now();

    if (_filterStatus == 'all') {
      // Tab "Semua" → hanya tampilkan pesanan hari ini
    ...
    if (_filterStatus == 'completed' || _filterStatus == 'cancelled') {
      // Tab Selesai & Batal → filter hari ini juga
      return _allOrders.where((o) =>
    ...
    // Tab lain (pending, processing, ready) → semua, tidak perlu filter tanggal
    // karena pesanan aktif bisa saja dari kemarin yang belum selesai
    return _allOrders.where((o) => o.status == _filterStatus).toList();
  }
  ...
  Future<void> _processAction(
    ...
    final confirmed = await _showActionDialog(
        title, subtitle, _getStatusColor(newStatus), icon);
    if (confirmed != true) return;

    _showSnackBar("Memproses pesanan ${order.orderNumber}...",
        icon: Icons.hourglass_top_rounded);

    final success =
        await OrderRemoteDatasource().updateOrderStatus(order.id, newStatus);

    if (!mounted) return;

    if (success) {
      _showSnackBar("Pesanan berhasil diupdate!",
          icon: Icons.check_circle_outline_rounded);

      // ✅ PERBAIKAN: Fetch dulu, baru pindah tab setelah data baru masuk
      // Ini mencegah tab kosong sebentar saat transisi
      await _fetchAllOrders();
      if (!mounted) return;

      // Pindah tab ke status baru (kecuali kalau dibatalkan, tetap di tab sekarang)
      if (newStatus != 'cancelled') {
        setState(() => _filterStatus = newStatus);
      }
    } else {
      _showSnackBar("Gagal memproses. Coba lagi.",
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(children: [
          ...
          Expanded(
              child: Text(message, style: GoogleFonts.poppins(fontSize: 12.5))),
        ]),
  ...
  Widget build(BuildContext context) {
    ...
    final orders = _filteredOrders; // ✅ Gunakan getter, bukan inline filter
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => const CashierCreateOrderPage()),
        ).then((value) {
          if (value == true) _fetchAllOrders();
        }),
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            SliverToBoxAdapter(child: _buildFilterSection(orders)),
            _isLoading
  ...
  Widget _buildStatCards() {
    final stats = [
      ...
      {
        'label': 'Diproses',
        'value': _countByStatus('processing').toString(),
        'icon': Icons.soup_kitchen_rounded,
        'color': Colors.blue.shade600
      },
      {
        'label': 'Siap',
        'value': _countByStatus('ready').toString(),
  ...
  Widget _buildStatCard({
    ...
    return Container(
      ...
      padding: EdgeInsets.symmetric(
          horizontal: isWide ? 16 : 10, vertical: isWide ? 14 : 12),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: isWide
          ? Row(children: [
              ...
              const Spacer(),
              // ✅ Tampilkan tanggal aktual di bawah badge "Hari ini"
              Column(
  ...
  // FILTER SECTION
  // ─────────────────────────────────────────────────
  // ✅ Terima orders sebagai parameter agar hitungan badge sesuai filter aktif
  Widget _buildFilterSection(List<Order> orders) {
    return Padding(
            ...
            if (!_isLoading)
              Container(
                ...
                decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(10)),
                // ✅ Badge count pakai orders yang sudah difilter
        ...
        children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(item.productName,
                                  ...
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis),
                              // ✅ Tampilkan badge Hot/Cold jika ada note
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Expanded(flex: 3, child: _buildActionButtons(order)),
            ]),
          ),
        ],
      ),
    ...
    return Row(
      ...
      children: [
        if (order.status == 'pending' || order.status == 'processing')
          GestureDetector(
            onTap: () => _processAction(order, 'cancelled', 'Batalkan Pesanan?',
            ...
            onTap: () {
              if (order.status == 'pending') {
                _processAction(
                  ...
                  'Terima pembayaran ${_formatPrice(order.totalAmount)} dan teruskan ke dapur?',
                ...
                _processAction(
                  order, 'ready',
                  'Pesanan Siap?',
                  'Tandai pesanan ini sudah selesai dimasak dan siap?',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Belum ada pesanan dengan\nstatus "${_getStatusLabel(_filterStatus)}".',
              textAlign: TextAlign.center,
              style: GoogleFonts.poppins(
                  fontSize: 13,
                  color: Colors.grey.shade500,
